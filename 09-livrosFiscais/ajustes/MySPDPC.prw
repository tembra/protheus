...
//            OBS: Conforme definição de conceito pelo Mauro Vulcão, em reunião
//                 realizada no dia 21/11/2013, na presença de Ricardo Medina,
//                 Maristela Lobato, Márcia Neri e Thieres Tembra.
...
EndIf
//inicializa ajuste
Processa({||TDSPDPC(lOpen)},'SPED PIS/COFINS','Lendo arquivo..')

Return Nil

/* ------------------- */

Static Function TDSPDPC(lOpen)
...
Else
	//se usuário esitver somente ajustando, exibe dialog para escolher arquivo
...
nTamI := Len(aTxt)
If nTamI == 0
	Alert('Impossível ler o arquivo origem. Verifique se você gerou o mesmo na pasta do servidor.')
	Return Nil
EndIf
...
cQry := " SELECT (FT_PRODUTO+FT_FILIAL) PRODUTO,"
cQry += "        FT_CST" + cTipo + " CST,"
cQry += "        FT_CFOP CFOP,"
cQry += "        SUM(FT_VALCONT) VALCONT,"
cQry += "        SUM(FT_BASE" + cTipo + ") BASE,"
cQry += "        FT_ALIQ" + cTipo + " ALIQ,"
cQry += "        SUM(FT_VAL" + cTipo + ") VALOR"
cQry += " FROM " + RetSqlName('SFT')
cQry += " WHERE FT_FILIAL = '" + cFil + "'"
cQry += "   AND FT_ENTRADA >= '" + _cDataI + "'"
cQry += "   AND FT_ENTRADA <= '" + _cDataF + "'"
cQry += "   AND LEFT(FT_CFOP,1) >= '5'"
cQry += "   AND FT_ESPECIE = 'CF'"
cQry += "   AND LEN(FT_DTCANC) = 0"
cQry += "   AND D_E_L_E_T_ <> '*'"
cQry += " GROUP BY (FT_PRODUTO+FT_FILIAL), FT_CST" + cTipo + ", FT_CFOP, FT_ALIQ" + cTipo
cQry += " ORDER BY (FT_PRODUTO+FT_FILIAL), FT_CST" + cTipo + ", FT_CFOP, FT_ALIQ" + cTipo
